...
async function initBuild() {
  ...
  function makeGradientTexture() {
    ...
    let Array
    const url = 'http://172.16.8.133:5173/img/222.png'
    const img = new Image()
    ...
    img.onload = function() {
      ...
      for (let y = 0; y < height; y++) {
        for (let x = 0; x < width; x++) {
          const index = x * y
          const r = imageData[0 + (width * y + x) * 4]
          const g = imageData[1 + (width * y + x) * 4]
          const b = imageData[2 + (width * y + x) * 4]
          // const a = imageData[3 + (width * y + x) * 4]
